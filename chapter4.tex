...
なお，実際に相手に送信する提案は\algref{alg:bidding}にしたがって決定される．なお，すでに受諾したまたは相手に受諾された提案を$bid_{accept}$とする．
相手に送信する提案は目標効用にかかわらず常に\algref{alg:bidding}によって作成される．そのため，常に$target(t,\alpha) > U_A(propose)$であるが
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \WHILE{$target(t,\alpha) > U_A(propose) \wedge \sum_{k = 1}^{N} propose[k][1] \neq 0$}
        ...
        \ENDWHILE
        \WHILE{$\sum_{k = 1}^{N} propose[k][1] \neq 0 \wedge \sum_{k = 1}^{N} propose[k][1] \geqq \sum_{k = 1}^{N} bid_t[k][1]$}
            \STATE $best_H = \displaystyle \min_{\{x | v_{xH} + v_{xA} \neq 0, 1 \leqq x \leqq N \} } J_H[x]$ 
            \STATE $propose[best_H] \leftarrow [temp[best_H][0], free[best_H] - 1, temp[best_H][2] + 1]$
            \STATE $free[best_H] \leftarrow free[best_H] - 1$
            \STATE $temp \leftarrow propose$
